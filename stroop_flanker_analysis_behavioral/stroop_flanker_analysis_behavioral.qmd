...
ggsave(p_all,file="plot_behavioral.pdf",width=24,height=15,units="cm")
```

# regressions per task: effects of congruency
...
```{r}
stroop <- stroop |> mutate(correct_num = ifelse(correct=="true",1,0))
```
...
lrtest(RI_id,RI_id_cong) #better

#final model
m_stroop_judgments <- RI_id_cong

summary(m_stroop_judgments)
...
m_flanker_rt <- ri_id_cong

summary(m_flanker_rt)
...
lrtest(RI_id_scen,RI_id_cong) #better

#final model
m_rules_judgments <- RI_id_cong

summary(m_rules_judgments)
