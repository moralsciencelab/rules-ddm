...
p1 <- clean %>% 
  ...
  labs(subtitle="Did this person comply with the rule?")+
...
"1) A mixed-effects logistic regression of rule compliance judgments, with text, purpose and the text\*purpose interactions as fixed effects (and scenarios and participants as random effects). We predict main (i.e., positive) effects of text and purpose."
...
exp(-1.55152) # -> more likely that people say yes to violate question

# effect of text violation:

exp(-3.54974) # -> more likely that people say yes to violate question, stronger effect than for purpose
...
"2) A mixed-effects regression of response times, with text, purpose, the text*purpose interaction, and response (yes vs. no) as fixed effects (and scenario and participant as random effects). -We predict an interaction effect of text*purpose (indicating longer response times on conflict trials). -We predict a main effect of response, with shorter reaction times for"no" than "yes" responses"
...
response <- lmer(rt ~ purpose + text + response + (1|subj_code) + (1|case), data = clean,na.action=na.exclude)

lrtest(purpose_text, response) #better

# interaction text x purpose

interaction <- lmer(rt ~ purpose*text + response + (1|subj_code) + (1|case), data = clean,na.action=na.exclude)
...
lrtest(interaction, interaction_resp) # better
```
