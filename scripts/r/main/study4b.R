...
learn2 = bind_rows(read.csv("data/learning2_batch1.csv"), 
                   read.csv("data/learning2_batch2.csv"))
glimpse(learn2)

## white text instructions
learn2 %>%
  filter(task == 'fixation') %>%
  group_by(run_id, subj_code, response) %>%
  tally() 

completes = learn2 %>%
  filter(!is.na(subj_code)) %>%
  group_by(run_id, subj_code) %>%
  ...
  select(run_id, subj_code, complete_time_elapsed)

failed_check = learn2 %>%
  filter(str_detect(stimulus, 'too many questions incorrectly')) %>%
  mutate(screen_out_time_elapsed = time_elapsed/1000/60) %>%
  select(run_id, subj_code, screen_out_time_elapsed)

id_list = full_join(completes, failed_check, by = c('run_id', 'subj_code')) 
#View(id_list)
...
learn2_clean = learn2 %>%
  ...
  select(run_id, subj_code, congruency, condition, trial, stimulus, response_num, rt) %>%
  full_join(., read.csv('data/stimulus_coding.csv'), by = 'stimulus') %>%
...
learn2_clean  %>%
  filter(subj_code %in% pass_list)

## T and P effects ----
model0 = glmer(response_num ~ (text + purpose) + (1 | subj_code) + (1 | rule), 
               subset(learn2_clean, !is.na(block) & subj_code %in% pass_list), family = 'binomial')
car::Anova(model0)
jtools::summ(model0)
...
mean_resps2 = learn2_clean %>%
  ungroup() %>%
  filter(as.numeric(rt) > 200, !is.na(block)) %>%
...
fig6a = ggplot(data = mean_resps2, aes(x = trial_norm, y = response_num)) + 
  ...
  geom_smooth(data = subset(learn2_clean, as.numeric(rt) > 500 & !is.na(block)), 
              aes(group = paste(case, block), fill = case, 
                  color = case, 
  ...
  scale_color_manual(values = c("c" = compliance_color, 
                                "u" = muted(compliance_color),
                                "o" = muted(violation_color), 
                                "v" = violation_color)) +
  scale_fill_manual(values = c("c" = compliance_color, 
                               "u" = muted(compliance_color),
                               "o" = muted(violation_color), 
                               "v" = violation_color))

ggsave('figures/Figure5a.png', dpi = 600, width = 16, height = 12, units = 'cm')
...
ddm_control = read.csv('results/model_learn2_control/model_learn2_control_within_traces.csv') %>%
...
ddm_stroop = read.csv('results/model_learn2_stroop/model_learn2_stroop_within_traces.csv') %>%
  select(-contains(c('_subj', '_std')), -X) %>%
  mutate(z = exp(z_trans)/(1 + exp(z_trans))) %>%
  select(-z_trans) %>%
  gather(key = 'item', value = 'value', na.rm = TRUE) %>%
...
ddm_rules = read.csv('results/model_learn2_rules/model_learn2_rules_within_traces.csv') %>%
  select(-contains(c('_subj', '_std')), -X) %>%
  mutate(z = exp(z_trans)/(1 + exp(z_trans))) %>%
  select(-z_trans) %>%
  gather(key = 'item', value = 'value', na.rm = TRUE) %>%
...
fig6c = ggplot(subset(ddm_fig4, parameter %in% c('Δa', 'Δt')), 
               aes(x = fct_rev(treatment), y = value, color = parameter)) +
  ...
  coord_flip() + 
  scale_color_manual(values = c(a_color, t_color)) +
  theme_classic() +
...
fig6b = ggplot(subset(ddm_fig4, parameter %in% c('Δv-text', 'Δv-purpose')), 
  ...
  geom_violin(aes(group = paste(parameter, treatment), fill = parameter), 
              trim = FALSE, draw_quantiles = c(.25, .75), alpha = .5, 
...
final_fig6 <- fig6a + fig6b + fig6c +
  plot_layout(widths = c(1.6, 1, 1.3)) +
  plot_annotation(tag_levels = list(c("A", "B", "")))

ggsave('figures/Figure5.png', 
       plot = final_fig6,
       dpi = 600, width = 26, height = 13, units = 'cm')


learn2_ddm <- 
  bind_rows(
  read_csv("results/model_learn2_control/model_learn2_control_output.csv") %>%
  rename(term = `...1`) %>%
  filter(!str_detect(term, "_subj."), !str_detect(term, "std")) %>%
  select(term, mean, `2.5q`, `97.5q`) %>% mutate(treatment = "Control"),
  read_csv("results/model_learn2_stroop/model_learn2_stroop_output.csv") %>%
    rename(term = `...1`) %>%
    filter(!str_detect(term, "_subj."), !str_detect(term, "std")) %>%
    select(term, mean, `2.5q`, `97.5q`)  %>% mutate(treatment = "Stroop"),
  read_csv("results/model_learn2_rules/model_learn2_rules_output.csv") %>%
    rename(term = `...1`) %>%
    filter(!str_detect(term, "_subj."), !str_detect(term, "std")) %>%
    select(term, mean, `2.5q`, `97.5q`)  %>% mutate(treatment = "Rules")) %>%
  arrange(term, treatment) 

print(learn2_ddm, n = Inf)
